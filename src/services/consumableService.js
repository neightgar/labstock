...
async function create(data, user = null) {
  ...
  return prisma.consumable.create({ data: payload, include: INCLUDE });
}
...
async function update(id, data, user = null) {
  ...
  return prisma.consumable.update({ where: { id }, data, include: INCLUDE });
}
...
async function softDelete(id, user = null) {
  await findById(id, user);
  return prisma.consumable.update({ where: { id }, data: { deletedAt: new Date() } });
}

async function restore(id) {
  return prisma.consumable.update({ where: { id }, data: { deletedAt: null } });
}
